...
#filenames: "NCBItrainset_corpus.txt", "NCBIdevelopset_corpus.txt", and "NCBItestset_corpus.txt"
#returns a list of instances in the desired format
def parse_NCBI_disease_corpus(filename):
    ...
    #now parse each instance into a list of the desired format
    paresed_corpus_list=[]
    for instance in entry_list:
        ...
        for mention in cache_entry[2:]: #the mentions are documented from the third line
            ...
            if int(cache_mention[1])<abstract_offset:
                cache_dict={'start':int(cache_mention[1]),'end':int(cache_mention[2]),'text':cache_mention[3],'type':cache_mention[4],'id':cache_mention[5]}
                title_mentions.append(cache_dict)
            else:
                cache_dict={'start':int(cache_mention[1])-abstract_offset,'end':int(cache_mention[2])-abstract_offset,'text':cache_mention[3],'type':cache_mention[4],'id':cache_mention[5]}
                abstract_mentions.append(cache_dict)
        sections=[{'text':title,'offset':0,'mentions':title_mentions},{'text':abstract,'offset':abstract_offset,'mentions':abstract_mentions}]
        paresed_corpus_list.append({'docid':docid,'sections':sections})
    return paresed_corpus_list
